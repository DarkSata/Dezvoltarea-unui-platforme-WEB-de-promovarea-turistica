...
export function Lightbox({ destination, onClose }: LightboxProps) {
  return (
    <div id="lightbox" className={`lightbox ${destination ? 'open' : ''}`} aria-hidden={!destination}>
      ...
      <div className="lightbox-panel" role="dialog" aria-modal="true" aria-label="Poză mărită">
        <button className="lightbox-close" type="button" aria-label="Închide" onClick={onClose}>
        ...
        <img
          ...
          alt={destination ? `Poză ${destination.name}` : 'Poză mărită'}
        />
